  ...
  return (
    <>
        <div className="grid grid-flow-col auto-cols-fr w-screen fixed bg-sky-500">
            {days.map((day) => (
                <Link key={day} className={`${pathname === getPathNameForDay(day.toLowerCase()) ? "bg-sky-200" : "text-white"} h-8 flex items-center justify-center w-100`} href={getPathNameForDay(day)}>{day}</Link>
            ))}
        </div>
